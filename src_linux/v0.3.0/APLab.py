...
class APLab(tk.Tk):
    ...
    def __init__(self, cnum, tnum, fs):
        ...
        # Define fonts
        self.small_font = tkFont.Font(root=self, family=gfont, size=self.small_fs)
        self.smallbold_font = tkFont.Font(root=self, family=gfont, size=self.small_fs, weight='bold')
        self.medium_font = tkFont.Font(root=self, family=gfont, size=self.medium_fs, weight='bold')
        self.large_font = tkFont.Font(root=self, family=gfont, size=self.large_fs, weight='bold')
    ...
    def loadData(self):
        ...
        setupWindow(self.topLoad, 300, 150)
    ...
    def changeCamera(self, restrict='no'):
        ...
        setupWindow(self.topCamera, 300, 350)
    ...
    def changeTelescope(self):
        ...
        setupWindow(self.topTelescope, 320, 350)
    ...
    def addIcon(self, window):
        ...
        try:
            img = ImageTk.PhotoImage(file=(os.getcwd() + '/aplab_icon.png'))
    ...
    def changeFS(self):
        ...
        fs_vals = [8, 9, 10, 11, 12, 13, 14, 15, 16]
...
class PlottingTool(ttk.Frame):
    ...
    def plot(self):
        ...
        if type == self.p1:
            ...
            self.ax.set_title(self.p1, name=gfont, weight='heavy', fontsize=medium_fs)
            self.ax.set_xlabel('Sub exposure time [s]', name=gfont, fontsize=small_fs)
            self.ax.set_ylabel('Target SNR', name=gfont, fontsize=small_fs)
            self.canvas.draw()
            ...
            self.ax.set_title(self.p2, name=gfont, weight='heavy', fontsize=medium_fs)
            self.ax.set_xlabel('Sub exposure time [s]', name=gfont, fontsize=small_fs)
            self.ax.set_ylabel('Stack SNR', name=gfont, fontsize=small_fs)
            self.canvas.draw()
            ...
            self.ax.set_title(self.p3, name=gfont, weight='heavy', fontsize=medium_fs)
            self.ax.set_xlabel('Number of subframes', name=gfont, fontsize=small_fs)
            self.ax.set_ylabel('Stack SNR', name=gfont, fontsize=small_fs)
            self.canvas.draw()
            ...
            self.ax.set_title(self.p4, name=gfont, weight='heavy', fontsize=medium_fs)
            self.ax.set_xlabel('Number of subframes', name=gfont, fontsize=small_fs)
            self.ax.set_ylabel('Stack SNR increase with one additional frame', name=gfont,
                               fontsize=small_fs)
            self.canvas.draw()
            ...
            for i in range(len(iso)):
                self.ax.annotate('%d x %d s' % (subs[i], np.ceil(exposure[i])), name=gfont,
            ...
            self.ax.text(0.5, 0.05,
                         ...
                         transform=self.ax.transAxes, name=gfont, fontsize=self.cont.tt_fs)
            
            self.ax.set_title(self.p5, name=gfont, weight='heavy', fontsize=medium_fs)
            self.ax.set_xlabel('ISO', name=gfont, fontsize=small_fs)
            self.ax.set_xticks(xvals)
            self.ax.set_xticklabels([str(i) for i in iso])
            self.ax.set_ylabel('Maximum stack SNR', name=gfont, fontsize=small_fs)
            self.canvas.draw()
            ...
            self.ax.set_title(self.p6, name=gfont, weight='heavy', fontsize=medium_fs)
            self.ax.set_xlabel('ISO', name=gfont, fontsize=small_fs)
            self.ax.set_xticks(xvals)
            self.ax.set_xticklabels([str(i) for i in iso])
            self.ax.set_ylabel('Dynamic range [stops]', name=gfont, fontsize=small_fs)
            self.canvas.draw()
            ...
            self.ax.plot((convSig(self.sf, True) if self.cont.lumSignalType.get() else self.sf),
                              current_snr, 'o', color='crimson', label='Current values', zorder=1)
            self.ax.legend(loc='best', numpoints=1, fontsize=small_fs)
            self.ax.plot((convSig(sf, True) if self.cont.lumSignalType.get() else sf), snr, '-',
                         color='crimson', zorder=0)
            self.ax.set_title(self.p7, name=gfont, weight='heavy', fontsize=medium_fs)
            self.ax.set_xlabel('Skyglow %s' % (u'[mag/arcsec\u00B2]' \
                                               if self.cont.lumSignalType.get() else '[e-/s]'),
                               name=gfont, fontsize=small_fs)
            self.ax.set_ylabel('Target SNR', name=gfont, fontsize=small_fs)
            self.canvas.draw()
            ...
            self.ax.cla()
            self.ax.plot((convSig(self.sf, True) if self.cont.lumSignalType.get() else self.sf),
                              current_dr, 'o', color='navy', label='Current values', zorder=1)
            self.ax.legend(loc='best', numpoints=1, fontsize=small_fs)
            self.ax.plot((convSig(sf, True) if self.cont.lumSignalType.get() else sf), dr, '-',
                         color='navy', zorder=0)
            self.ax.set_title(self.p8, name=gfont, weight='heavy', fontsize=medium_fs)
            self.ax.set_xlabel('Skyglow %s' % (u'[mag/arcsec\u00B2]' \
                                               if self.cont.lumSignalType.get() else '[e-/s]'),
                               name=gfont, fontsize=small_fs)
            self.ax.set_ylabel('Dynamic range [stops]', name=gfont, fontsize=small_fs)
            ...
            self.ax.set_title(self.p9, name=gfont, weight='heavy', fontsize=medium_fs)
            self.ax.set_xlabel('Sub exposure time [s]', name=gfont, fontsize=small_fs)
            self.ax.set_ylabel('Dynamic range [stops]', name=gfont, fontsize=small_fs)
            self.canvas.draw()
            ...
            self.ax.cla()
            self.ax.plot((convSig(self.tf, True) if self.cont.lumSignalType.get() else self.tf),
                              current_snr, 'o', color='crimson', label='Current values', zorder=1)
            self.ax.legend(loc='best', numpoints=1, fontsize=small_fs)
            self.ax.plot((convSig(tf, True) if self.cont.lumSignalType.get() else tf), snr, '-',
                         color='crimson', zorder=0)
            self.ax.set_title(self.p10, name=gfont, weight='heavy', fontsize=medium_fs)
            ...
            self.ax.plot(np.log10(gain)/np.log10(2.0), np.log10(sat_cap)/np.log10(2.0), '-o',
                         color='darkviolet')
            self.ax.set_title(self.p11, name=gfont, weight='heavy', fontsize=medium_fs)
            self.ax.set_xlabel('log2(gain)', name=gfont, fontsize=small_fs)
            self.ax.set_ylabel('log2(saturation capacity)', name=gfont, fontsize=small_fs)
            self.canvas.draw()
            ...
            self.ax.set_title(self.pc1 + ' comparison plot', name=gfont, weight='heavy', 
                              fontsize=medium_fs)
            self.ax.set_xlabel('Sub exposure time [s]', name=gfont, fontsize=small_fs)
            self.ax.set_yticks([0, 0.2, 0.4, 0.6, 0.8, 1])
            self.ax.set_yticklabels(['Min', '', '', '', '', 'Max'])
            self.ax.set_ylabel('Value', name=gfont, fontsize=small_fs)
            self.canvas.draw()
            ...
            self.ax.legend(loc='best', fontsize=small_fs)
            self.ax.set_title(self.pc2 + ' comparison plot', name=gfont, weight='heavy', 
                              fontsize=medium_fs)
            self.ax.set_xlabel('Number of subframes', name=gfont, fontsize=small_fs)
            self.ax.set_yticks([0, 0.2, 0.4, 0.6, 0.8, 1])
            self.ax.set_yticklabels(['Min', '', '', '', '', 'Max'])
            ...
            self.ax.set_title(self.pc3 + ' comparison plot', name=gfont, weight='heavy', 
                              fontsize=medium_fs)
            self.ax.set_xlabel('Skyglow %s' % (u'[mag/arcsec\u00B2]' \
                                               if self.cont.lumSignalType.get() else '[e-/s]'), 
                               name=gfont, fontsize=small_fs)
            ...
            self.ax.legend(loc='best', fontsize=small_fs)
            self.ax.set_title(self.pc4 + ' comparison plot', name=gfont, weight='heavy', 
                              fontsize=medium_fs)
            self.ax.set_xlabel('ISO', name=gfont, fontsize=small_fs)
            self.ax.set_xticks(xvals)
            self.ax.set_xticklabels([str(i) for i in iso])
...
class ImageAnalyser(ttk.Frame):
    ...
    def getImage(self, label, filepath, filename, type, splitCFA=False, compare=False):
        ...
        if '*.' + filename.split('.')[1].lower() in self.supportedformats[0][1]:
            ...
            tiff_filepath = '.'.join(filepath.split('.')[:-1]) + '.tiff'
        ...
        if splitCFA:
            ...
            if self.cont.isDSLR and not filename.split('.')[-1].lower() in ['tif', 'tiff', 'fit', 'fits']:
                ...
                if self.CFAPattern is None:
                    for line in metadata.split('\n'):
                        ...
                        if parts[0] == 'Filter pattern': self.CFAPattern = parts[1].strip()[:4]
            ...
            elif not self.CFAPattern in ['RGGB', 'BGGR', 'GRBG', 'GBRG']:
                self.varMessageLabel.set('Detected CFA pattern %s not recognized.' % (self.CFAPattern))
                self.labelMessage.configure(foreground='crimson')
                self.busy = True
                self.askCFAPattern()
                self.wait_window(self.topAskCFA)
    ...
    def updateDisplayedImage(self, label, fromHist=False):
        ...
        plt.imsave(self.labelNames[label] + '.jpg', label.stretched_img, cmap=plt.get_cmap('gray'), 
                   vmin=0, vmax=65535)
...
class ToolTip:
    ...
    def showToolTip(self, tiptext):
        ...
        label = tk.Label(self.topTip, text=tiptext, justify='left', background='white',
                         relief='solid', borderwidth=1, font=(gfont, self.fs))
...
class ErrorWindow(tk.Tk):
    ...
    def __init__(self, error_message):
        ...
        errfont = tkFont.Font(root=self, family=gfont, size=9)
...
DEFAULT_BG = '#%02x%02x%02x' % (217, 217, 217)

# Window sizes

l_x = 1050 # Largest width
l_y = 610  # Largest height
...
SIM_WINDOW_SIZE = (l_x, 580)
PLOT_WINDOW_SIZE = (l_x, 580)
