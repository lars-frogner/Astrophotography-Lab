...
from aplab_image_solver import ImageSolver
...
class ToolManager(tk.Tk):
    ...
    def __init__(self, cnum, tnum, fs):
        ...
        self.title('Astrophotography Lab 0.5.0') # Set window title
        ...
        menuTool.add_checkbutton(label='Image Solver', variable=self.solvMode,
                                 command=self.enterSolvFrame)
        ...
        frameSolv = ImageSolver(self.container, self)
        ...
        self.frames[ImageSolver] = frameSolv
        ...
        frameSolv.grid(row=0, column=0, sticky='NSEW')
    ...
    def enterPlotFrame(self):
        ...
        # Close any open toplevels
        try:
            self.frames[ImageSimulator].topCanvas.destroy()
            self.frames[FOVCalculator].topCanvas.destroy()
        except:
            pass
    ...
    def enterSolvFrame(self):
    
        '''Shows the Image Solver frame.'''
    
        # Do nothing if already in FOV frame
        if not self.calMode.get() and not self.simMode.get() \
           and not self.plMode.get() and not self.anMode.get() \
           and not self.fovMode.get():
            self.solvMode.set(1) # Keep state from changing
            return None
         
        solvframe = self.frames[ImageSolver]
         
        solvframe.varMessageLabel.set('') # Clear message label
        self.showFrame(ImageSolver)
        ...
        # Resize and re-center window
        apc.setupWindow(self, *C.SOLV_WINDOW_SIZE)
