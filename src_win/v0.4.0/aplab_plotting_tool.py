...
class PlottingTool(ttk.Frame):
    ...
    def processInput(self):
        ...
        try:
            if self.useFlux:
                self.sf = (apc.convSig(self.varSF.get(), False) if self.cont.lumSignalType.get() \
                                                            else self.varSF.get())
        ...
        try:
            if self.useLim:
                self.lf = (apc.convSig(self.varLF.get(), False) if self.cont.lumSignalType.get() \
                                                            else self.varLF.get())
        ...
        try:
            if self.useFlux:
                self.tf = (apc.convSig(self.varTF.get(), False) if self.cont.lumSignalType.get() \
                                                            else self.varTF.get())
        ...
        if type == self.p1:
            ...
            if self.cont.lumSignalType.get():
                ...
                if f < 20:
                    ...
                    min = 19
                    max = 22
                sf = apc.convSig(np.linspace(max, min, 200), False)
            else:
            ...
            self.ax.cla()
            self.ax.plot((apc.convSig(sf, True) if self.cont.lumSignalType.get() else sf), snr, '-',
                         color='crimson')
            p, = self.ax.plot((apc.convSig(self.sf, True) if self.cont.lumSignalType.get() else self.sf),
            ...
            self.ax.cla()
            self.ax.plot((apc.convSig(sf, True) if self.cont.lumSignalType.get() else sf), dr, '-',
                         color='navy')
            p, = self.ax.plot((apc.convSig(self.sf, True) if self.cont.lumSignalType.get() else self.sf),
            ...
            if self.cont.lumSignalType.get():
                ...
                if f < 20:
                    ...
                    min = 19
                    max = 22
                tf = apc.convSig(np.linspace(max, min, 200), False)
            else:
            ...
            self.ax.cla()
            self.ax.plot((apc.convSig(tf, True) if self.cont.lumSignalType.get() else tf), snr, '-',
                         color='crimson')
            p, = self.ax.plot((apc.convSig(self.tf, True) if self.cont.lumSignalType.get() else self.tf),
                              current_snr, 'o', color='crimson', label='Current values')
            self.ax.legend(handles=[p], loc='best', numpoints=1, fontsize=small_fs)
            ...
            self.ax.cla()
            self.ax.plot((apc.convSig(sf, True) if self.cont.lumSignalType.get() else sf), snr1, '-', 
                         color='crimson', label='Target SNR: %.1f - %.1f' % (snr1_min, snr1_max))
            self.ax.plot((apc.convSig(sf, True) if self.cont.lumSignalType.get() else sf), dr1, '-', 
                         color='forestgreen', 
                         label='Dynamic range: %.1f - %.1f stops' % (dr1_min, dr1_max))
            self.ax.legend(loc='best', fontsize=small_fs)
            self.ax.plot((apc.convSig(self.sf, True) if self.cont.lumSignalType.get() else self.sf),
