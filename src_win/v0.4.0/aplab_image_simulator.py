...
class ImageSimulator(ttk.Frame):
    ...
    def getSuggestedSettings(self):
        ...
        try:
            df = self.varDF.get()
            sf = (apc.convSig(self.varSF.get(), False) if self.cont.lumSignalType.get() \
                                                            else self.varSF.get())
            tf = (apc.convSig(self.varTF.get(), False) if self.cont.lumSignalType.get() \
                                                        else self.varTF.get())
        ...
        try:
            lf = (apc.convSig(self.varLF.get(), False) if self.cont.lumSignalType.get() \
                                                            else self.varLF.get())
        except ValueError:
            self.varLF.set('')
            lf = tf
